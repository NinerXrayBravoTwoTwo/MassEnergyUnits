"""
minkowski_entropy.spacetime_entropy
===================================
...
# Light-travel distance in one second (matches massenergy.units: s = 2.99792458e8 m)
...
def v_at_distance(
    ...
    """V at distance d (c=1); lambda_scale is T_double or T_half."""
    if v0 <= 0:
        raise ValueError("v0 must be positive")
    if lambda_scale <= 0:
        raise ValueError("lambda_scale must be positive")
...
if __name__ == "__main__":
    print("=== minkowski_entropy.spacetime_entropy self-test ===\n")
    ...
    print(f"Light-second scale: {LIGHT_SECOND_KM} km (matches massenergy.units)")
    d_scaled = 14.0  # example scaled distance (units of lambda)
